...
*Setup instructions for a Gitian build of IberiKas Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the IberiKas
...
- [Building IberiKas Core](#building-iberikas-core)
...
Clone the git repositories for iberikas and Gitian.
...
in the iberikas repository under 'Fetch and create inputs' to install sources which require
...
To build IberiKas Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the iberikas repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/iberikas/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/iberikas-project/iberikas
...
For example:
```bash
URL=https://github.com/thrasher-/iberikas.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit iberikas=${COMMIT} --url iberikas=${URL} ../iberikas/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit iberikas=${COMMIT} --url iberikas=${URL} ../iberikas/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit iberikas=${COMMIT} --url iberikas=${URL} ../iberikas/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the iberikas git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../iberikas/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/iberikas
SIGPATH=/some/root/path/iberikas-detached-sigs

./bin/gbuild --url iberikas=${BTCPATH},signature=${SIGPATH} ../iberikas/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/iberikas-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/iberikas-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/iberikas-osx-build.assert
```
...
[iberikas-project/gitian.sigs.ltc](https://github.com/iberikas-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
